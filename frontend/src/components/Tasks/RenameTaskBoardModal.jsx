...
export default function RenameTaskBoardModal({ board, onClose, onSubmit, loading }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60 px-4">
      <div className="bg-card border border-border rounded-lg p-6 w-full max-w-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-foreground font-semibold">{t('tasks.rename')}</h3>
          <button
            onClick={onClose}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {loading ? t('tasks.saving') : t('tasks.rename')}
            </button>
          </div>
        </form>
      </div>
