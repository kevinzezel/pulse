...
export default function RenameSessionModal({ session, onClose, onSubmit, loading }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60 px-4">
      <div className="bg-card border border-border rounded-lg p-6 w-full max-w-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-foreground font-semibold">{t('modal.renameSession.title')}</h3>
          <button
            onClick={onClose}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit}>
          <label className="block text-sm text-muted-foreground mb-1">
            {t('modal.renameSession.nameLabel')}
          </label>
          <input
            type="text"
            value={name}
            ...
            placeholder={t('modal.renameSession.placeholder')}
            maxLength={50}
            autoFocus
            disabled={loading}
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {loading ? t('modal.renameSession.saving') : t('modal.renameSession.save')}
            </button>
          </div>
        </form>
      </div>
