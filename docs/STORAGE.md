...
Pulse persists ten JSON documents per dashboard: `projects`, `flows`, `groups`, `notes`, `prompts`, `servers`, `layouts`, `view-state`, `sessions`, `compose-drafts`. By default they live in local files under `frontend/data/`. Configure a remote driver to share the same workspace across machines — install Pulse on your desktop, laptop, and a VPS, point each one at the same remote, and the same data appears on all three.
