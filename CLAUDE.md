...
- **`client/src/tools/pty.py`** — `PTYSession` encapsulates process + master_fd + scrollback (bytearray, 512 KB with head trim). Registry is separate from the metadata dict to avoid non-serializable objects in JSON. Shells are spawned with the env produced by `build_pty_env()` — it starts from `os.environ`, strips any key found in Pulse env files (`client/.env`, `frontend/.env`, `~/.config/pulse/client.env`, `~/.config/pulse/frontend.env`), and also strips fallback internal keys (`COMPOSE_PROJECT_NAME`, `VERSION`, `API_*`, `WEB_*`, `AUTH_*`, `TLS_*`, anything prefixed `PULSE_`). Toolchain envs (`PATH`, `HOME`, `SHELL`, `SSH_AUTH_SOCK`, `DISPLAY`, `LANG`, `AWS_*`, `GOOGLE_*`, etc.) are preserved.
...
Single-password gate + JWT HS256 24h in an httpOnly cookie `rt_auth`. `AUTH_PASSWORD` is the shared password; `AUTH_JWT_SECRET` is auto-generated by `start.sh` if it's `change-me` or missing. `/login` and `/api/auth/*` are the only public routes — `src/middleware.js` protects everything else (UI + API). Each API route handling sensitive data (`/api/servers|groups|prompts`) is also wrapped by `withAuth()` from `@/lib/auth` (defense-in-depth / DAL). The middleware still accepts the legacy `rt:auth` cookie name for compatibility, but new logins set `rt_auth`.
...
**Preview-first by default.** Pulse has two release channels: stable (`vX.Y.Z`) and preview (`vX.Y.Z-pre`). Releases generated by AI must always use the **preview** suffix `-pre`, even for hotfixes — preview tags are flagged as GitHub prereleases by `.github/workflows/release.yml` and do not surface in `pulse upgrade` or the dashboard's update modal until users opt in with `--preview`. A plain stable tag (`vX.Y.Z` without `-pre`) is only published when the repo owner explicitly asks for the promotion ("ship a stable release", "promote 2.5.0-pre to stable"). When in doubt, default to `-pre`.

The block must contain, in this order:

1. **Suggested version bump** following SemVer (always with `-pre` suffix unless promoting to stable):
   - **patch** (`X.Y.Z+1-pre`) — only bug fixes, no observable behavior change beyond the fix
   - **minor** (`X.Y+1.0-pre`) — new features, new CLI commands, non-breaking UI changes, API expansions
   - **major** (`X+1.0.0-pre`) — breaking changes (renamed command, removed flag, env/config schema change, incompatible API change)
2. **`CHANGELOG.md` update** done by you (Claude) before closing the task — new section at the top (`## [X.Y.Z-pre] — YYYY-MM-DD`) with appropriate `### Added` / `### Changed` / `### Fixed` / `### Removed`, descriptive bullets per change, and updated footer links pointing at the `-pre` tag. If you haven't done this yet, do it before returning the block. **Always write CHANGELOG entries in English.**
3. **Ready-to-run shell script in `/tmp/`** — instead of returning a block of commands to copy/paste, **generate an executable file** via the Write tool at `/tmp/pulse-release-v<X.Y.Z>-pre.sh` (preview release) or `/tmp/pulse-commit-<slug>.sh` (docs-only / commit without tag), with implicit exec permission (the user runs `bash /tmp/...sh`). Default release content:
   ...
   git tag -a v<X.Y.Z>-pre -m "Pulse v<X.Y.Z>-pre — <summary>"
   git push origin main
   git push origin v<X.Y.Z>-pre  # triggers .github/workflows/release.yml; published as GitHub prerelease
   ...
   When the owner explicitly promotes to stable, drop the `-pre` suffix everywhere (script path, tag name, CHANGELOG section). Stable promotion typically reuses the `X.Y.Z` core that was last shipped as `X.Y.Z-pre`.

4. **Final message** mentioning the script path (e.g. "Run: `bash /tmp/pulse-release-v1.3.3-pre.sh`") and what the script will do, in one line.
